...
12. **Importação da base MongoDB do Izalci — Fase 1 concluída, Fases 2-5 não iniciadas** (18/07): `docs/superpowers/specs/2026-07-18-importacao-izalci-mongodb-design.md` — trazer os dados reais do senador Izalci Lucas (hoje em MongoDB Atlas, ~142 mil pessoas) para um gabinete novo (`IZALCI`), mantendo o gabinete de testes atual intacto. Backup físico completo já realizado (`/Users/renato/Backups/mongodb-meubancodedadosprod-2026-07-18/`, verificado íntegro). Spec cobre mapeamento de campos completo, hierarquia nova de Região (cidade→bairro via `regiaoPaiId`), 2 campos novos em `Pessoa` (zona/seção eleitoral), estrutura de telefone extra, reconstrução de rede de indicação (`VinculoRede`, excluindo 2 contas de desenvolvedor do sistema antigo identificadas), e importação de Banco de Talentos a partir de currículos. Dividido em 5 fases, cada uma com plano formal próprio antes de codar. **Fase 1 (fundação de schema) implementada em 18/07 — ver seção 24.** Duas funcionalidades ficaram para um sprint futuro (fora deste projeto): botão "adicionar telefone" na UI, e ferramenta de mesclar regiões com nome parecido/digitado errado. Gabinete IZALCI ainda não foi criado (ação manual pendente do usuário).
