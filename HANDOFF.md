...
- **Bug pré-existente encontrado no smoke test manual pós-deploy (não introduzido por esta feature)**: o cadastro público nunca completava quando o `<input type="file">` de foto ficava vazio (comum — sem foto é o caso mais frequente) — um `File` vazio embutido no objeto que `submeterCadastro` recebe quebra a serialização da Server Action do Next ("Only plain objects... Classes or null prototypes are not supported"). **Fix parcial aplicado** (`3310c45`): só inclui `foto` no payload quando um arquivo de verdade foi escolhido — resolve o caso sem foto. **Upload de foto real no cadastro público continua quebrado** (mesmo erro ocorre com qualquer `File`, vazio ou não) — ver pendência 10.
...
10. **Upload de foto no cadastro público continua quebrado** (achado em 12/07, durante smoke test manual pós-deploy da seção 15 — bug pré-existente, não introduzido nesta sessão). Qualquer `File` (vazio ou com conteúdo real) embutido no objeto comum que `submeterCadastro` recebe quebra a serialização da Server Action do Next ("Only plain objects... Classes or null prototypes are not supported"), porque a chamada é manual (`await submeterCadastro({...})`), fora do padrão nativo `<form action={fn}>`. O caso sem foto (mais comum) já foi corrigido (`3310c45` — só inclui `foto` no payload quando um arquivo de verdade foi escolhido). **O caso com foto real continua 100% quebrado** — precisa reestruturar `submeterCadastro` pra receber um `FormData` nativo como argumento único, igual `uploadFotoPessoa` (upload de foto na ficha do admin, que funciona) já faz, ou separar em duas chamadas (criar pessoa, depois upload). **Requer brainstorming/plano antes de código** (decisão do usuário, 12/07/2026 — mudança estrutural na action, não um ajuste de uma linha).
