...
- 2 achados Minor da revisão final, não bloqueantes, sem fix aplicado ainda: (1) `findFirst` case-insensitive vs. índice case-sensitive em `criar-area-demanda`/`criar-profissao`/`criar-regiao` — quase-duplicata por case ainda possível sob corrida real; (2) comentário de índice parcial no `schema.prisma` presente só em `Segmento`, ausente em `Profissao`/`Regiao` (mesma técnica, inconsistência de documentação).
