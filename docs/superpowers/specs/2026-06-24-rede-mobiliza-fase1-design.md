...
**Modo suporte do super-admin:** quando o super-admin clica em "Entrar em modo suporte" para um gabinete específico, o servidor define o cookie `suporteSessao` com atributos `httpOnly=true, secure=true, sameSite='strict', path='/'` contendo um payload JSON `{ gabineteId, sessaoId }`. **`path='/'` é obrigatório** — com `path='/super-admin'` o browser não enviaria o cookie para rotas `/api/...` usadas pelas queries Prisma de dados do gabinete. `secure=true` garante transmissão apenas via HTTPS. **Serialização/desserialização do cookie:** ao gravar: `cookies().set('suporteSessao', JSON.stringify({ gabineteId, sessaoId }), { httpOnly: true, secure: true, sameSite: 'strict', path: '/' })`; ao ler:
```typescript
const raw = cookies().get('suporteSessao')?.value
if (!raw) { /* cookie ausente — não está em modo suporte */ }
const { gabineteId, sessaoId } = JSON.parse(raw)
if (!gabineteId || !sessaoId) throw new Error('cookie suporteSessao malformado')
```
Use `?.value` (não `!.value`) para evitar TypeError quando o cookie está ausente; após o parse, valide os campos antes de usá-los. As queries Prisma dentro do modo suporte usam o `gabineteId` extraído desse JSON — não de `UsuarioGabinete`. **Todo handler que lê o cookie `suporteSessao` deve primeiro verificar que `session.user.app_metadata.role === 'super-admin'` — se o role for diferente, o cookie deve ser ignorado mesmo que presente (previne que um admin normal com cookie residual acesse dados de outro gabinete).** A regra "nunca de parâmetros de URL" (linha acima) aplica-se a admins e mobilizadores; o super-admin em modo suporte usa o mecanismo de cookie descrito aqui. **Entrada no modo suporte:** o servidor gera um `sessaoId` (cuid), cria `LogSuporte` com `acao="acesso_inicio"` e define o cookie. **Saída do modo suporte:** quando o super-admin clica em "Sair do modo suporte", o servidor remove o cookie, cria `LogSuporte` com `acao="acesso_fim"` preenchendo `saidoEm`, e invalida o `sessaoId`. Se o browser fechar sem logout explícito, o cookie expira com a sessão do browser (cookie de sessão, sem `max-age`); a ausência do cookie no próximo acesso a `/super-admin/` indica que não há sessão de suporte ativa — o `LogSuporte` ficará com `saidoEm=null` (sessão "aberta"), detectável via a query descrita abaixo no modelo `LogSuporte`.
